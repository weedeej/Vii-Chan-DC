...
export async function waitlist(interaction, skinlist) {
	...
	const whitelist = resp.data.waitList;
	...
	for (let i = 0; i < whitelist.length; i++) {
		...
		const currentSkin = skinlist.data[whitelist[i]];
		currentRow.addComponents(
			new MessageButton()
				.setLabel(`[ ${i + 1} ]`)
				.setCustomId(`remove_${whitelist[i]}_${senderID}`)
				.setStyle('DANGER'),
		);
		...
		Logger.info(`Button num: ${i}: remove_${whitelist[i]}_${senderID}`, senderID);
		buttonCount++;
	}
